...
export function PendingOrderPaymentModal({
  ...
  return (
    <BottomPopup visible={visible} onClose={onClose} title="待支付订单">
      <div className="rounded-[24px] border border-[#fbd005]/18 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.03))] p-4 shadow-[0_16px_36px_rgba(0,0,0,0.16)]">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[#ffe08a]">Payment Summary</p>
            <p className="mt-2 text-sm text-white/58">当前预订单会根据接口返回的支付方式决定是系统余额支付还是链上支付。</p>
          </div>
          <div className="rounded-full bg-[#fbd005]/12 px-3 py-1 text-[11px] font-semibold text-[#ffe08a]">实时判定</div>
        </div>

        <div className="space-y-2">
          {summaryRows.map((item) => (
            <div key={item.label} className="flex items-center justify-between rounded-2xl bg-black/20 px-3 py-3 text-sm">
        ...
        {paymentMethods.map((method) => {
          ...
          return (
            <button
              ...
              className={`w-full rounded-[24px] border px-4 py-4 text-left transition ${selected ? 'border-[#fbd005]/70 bg-[#fbd005]/10 shadow-[0_14px_30px_rgba(251,208,5,0.12)]' : 'border-white/10 bg-white/5'} ${disabled ? 'opacity-60' : ''}`}
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex min-w-0 gap-3">
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-[#fbd005]/12">
                  ...
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full bg-white/8 px-2.5 py-1 text-[11px] font-medium text-white/58">{badgeText}</span>
                      {method.targetChainName ? (
                        <span className="rounded-full bg-sky-400/10 px-2.5 py-1 text-[11px] font-medium text-sky-200">{method.targetChainName}</span>
                      ) : null}
                    </div>
                ...
                <div className="text-right">
                  <span className={`mt-1 flex h-6 w-6 items-center justify-center rounded-full border text-[11px] ${selected ? 'border-[#fbd005] bg-[#fbd005] text-black' : 'border-white/20 text-transparent'}`}>
                    ✓
                  </span>
                  <div className="mt-4 text-[11px] text-white/32">
                    {method.needSwitchChain
                      ? '需切链'
                      : method.available || method.type === 2
                        ? '可支付'
                        : '余额不足'}
                  </div>
                </div>
      ...
      {selectedMethod ? (
        <div className="mt-4 rounded-[22px] border border-white/8 bg-black/20 px-4 py-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-sm font-semibold text-white">{selectedMethod.name}</div>
            ...
            <span className="rounded-full bg-white/8 px-3 py-1 text-[11px] font-medium text-white/56">
      ...
      <button
        ...
        className="mt-4 w-full rounded-2xl bg-gradient-to-r from-[#fff193] to-[#eba500] py-3.5 text-sm font-bold text-black shadow-[0_16px_28px_rgba(235,165,0,0.24)] disabled:opacity-40"
      >
        确认支付
      </button>
    </BottomPopup>
