...
export function PendingOrderPaymentModal({
  ...
  return (
    <BottomPopup visible={visible} onClose={onClose} title={copy.pendingOrder}>
      ...
      <div className="mt-4 space-y-3">
        {paymentMethods.map((method) => {
          ...
          const badgeText = method.type === 1 ? copy.systemBalance : copy.onChainPayment
          return (
            <button
              key={method.id}
              type="button"
                  ...
                  <div className="min-w-0">
                    ...
                    <div className={`mt-1.5 text-[12px] leading-5 ${method.needSwitchChain ? 'text-amber-300' : !method.available && method.type === 1 ? 'text-red-300' : 'text-white/58'}`}>
                      {method.needSwitchChain
                        ? fillTemplate(copy.networkMismatch, { network: method.targetChainName || '' })
                        : !method.available && method.type === 1
                          ? fillTemplate(copy.insufficientBalance, { amount: Number(method.balance || 0).toFixed(6) })
                          : fillTemplate(copy.availableBalance, { amount: Number(method.balance || 0).toFixed(6) })}
                    </div>
                  </div>
                </div>
            ...
            <div>
              ...
              <div className="mt-1 text-[12px] leading-5 text-white/45">
                {selectedMethod.type === 1
                  ? copy.serverBalancePaymentHint
                  : selectedMethod.needSwitchChain
                    ? fillTemplate(copy.switchToNetwork, { network: selectedMethod.targetChainName || '' })
                    : copy.walletPaymentHint}
              </div>
            </div>
